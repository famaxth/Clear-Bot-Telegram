...
def bot_command_start(message):
    ...
    bot.send_message(
        message.chat.id, config.texts['start'] + config.texts["help_text"], reply_markup=keyboard)
        ...
        try:
            if text_save.lower() in config.texts['add_chat_list']:
                ...
                button = telebot.types.InlineKeyboardButton(
                    text='Выбрать чат...', url=f't.me/{bot_info.username}?startgroup=true')
                keyboard.add(button)
    ...
    for word in text:
        ...
        if result:
            try:
                ...
                text = ''
                bot_chat_info = bot.get_chat_member(
                    message.chat.id, bot_info.id)
                if not bot_chat_info.can_delete_messages:
                    ...
                    bot.send_message(
                        message.chat.id, config.texts['no_perm'].format(text))
            except Exception as e:
                logger.warning(
                    f'Неизвестная ошибка: {e}', name=message.from_user.first_name)
...
def bot_join(message):
    for user in message.new_chat_members:
        if user.id == bot_info.id:
            logger.info(
                f'Пригласили в группу {message.chat.title}', name=message.from_user.first_name)
...
def bot_left(message):
    ...
    if user.id == bot_info.id:
        logger.info(
            f'Выкинули из группы {message.chat.title}', name=message.from_user.first_name)
